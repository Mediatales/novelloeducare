...
const Universityhero = ({eligibilityPoints , mbbsBenefits , advantages , countryData}) => {
  return (
    <div className="p-6 md:p-12">
      ...
      <h2 className="text-center text-2xl md:text-3xl font-bold mb-6">Why to choose MBBS in {countryData?.country.charAt(0).toUpperCase() + countryData?.country.slice(1)}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
  ...
  Key advantages of pursuing MBBS in {countryData?.country.charAt(0).toUpperCase() + countryData?.country.slice(1)}
</h2>
<div className="flex flex-col md:flex-row items-center w-full  justify-center gap-6">
  ...
  <img
    ...
    alt={`MBBS in ${countryData?.country.charAt(0).toUpperCase() + countryData?.country.slice(1)}`}
